...
app.kubernetes.io/part-of: comqtt-dashboard
...
{{/*
Resolve which broker binary to invoke. The dashboard add-on ships its own
single-mode and cluster-mode binaries under different names than upstream.
*/}}
{{- define "comqtt.binary" -}}
{{- if eq .Values.mode "cluster" -}}/comqtt-cluster-dashboard{{- else -}}/comqtt-dashboard{{- end -}}
{{- end -}}
...
{{/*
Whether the chart should render its own dashboard Secret (i.e. at least one
of the inline values is set and no existingSecret overrides it).
*/}}
...
configured. The dashboard add-on serves the dashboard unconditionally, so
there is no `enabled` guard.
*/}}
